...
- last_registry_smoke: 2026-05-25 `servo-installer` (`0.5.6`, `latest`) passed with `--skip-remote`
...
2026-05-25 post-publish verification for `servo-installer` passed in `--skip-remote` mode after `0.5.6` publication. Evidence was kept in a temporary smoke directory and only the selector/version outcome is retained here as long-term fact. The smoke used `latest -> 0.5.6`, temporary targets passed, and no remote mutation was performed.
