...
- last_registry_smoke: 2026-05-26 `servo-installer` (`0.5.7`, `latest`) passed with `--skip-remote`
...
2026-05-26 post-publish verification for `servo-installer` passed in `--skip-remote` mode after `0.5.7` publication. Evidence was kept in `/tmp/servo-installer-registry-npx-smoke-YYlZvA/report.md` during release closeout, and only the selector/version outcome is retained here as long-term fact. The smoke used `latest -> 0.5.7`, temporary targets passed, and no remote mutation was performed.
