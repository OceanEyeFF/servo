...
- **closeout pipeline / cleanup 合约变更**：先跑 `governance_semantic_check.py`；再检查 harness-skill §10.7 中 closeout pipeline 是否完整包含 `Self-Review → Single-Acceptance → Closeout Gate → PR → Merge → Doc-Catch-Up → Refresh → Cleanup → return to RepoScope`，以及 self-review / single-acceptance 合约是否引用了 [self-review-contract.md](../../harness/artifact/worktrack/self-review-contract.md) 和 [single-acceptance-contract.md](../../harness/artifact/worktrack/single-acceptance-contract.md)。Cleanup 不碰 remote、不删 unmerged 分支、不删 baseline，失败不阻断 acceptance。Closeout record 合约见 `docs/harness/artifact/worktrack/README.md`
