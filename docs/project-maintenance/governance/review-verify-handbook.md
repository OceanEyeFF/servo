...
- **closeout pipeline / cleanup 合约变更**：先跑 `governance_semantic_check.py`；再检查 harness-skill §10.7 中 closeout pipeline 是否完整包含 `merge → refresh → cleanup → return to RepoScope`，以及 Milestone final acceptance 后是否绑定 `servo-cleanup-skill`。Cleanup 不碰 remote、不删 unmerged 分支、不删 baseline，失败不阻断 acceptance。Closeout record 合约见 `docs/harness/artifact/worktrack/README.md`
