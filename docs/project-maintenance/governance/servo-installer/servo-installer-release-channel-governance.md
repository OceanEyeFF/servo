...
2026-05-25，本地 source tuple 已发布为 `v0.5.6` 的 `latest` channel release：

- root `package.json` version：`0.5.6`
- local scaffold `toolchain/scripts/deploy/package.json` version：`0.5.6`
- approval lock：`approvedVersion=0.5.6`、`approvedGitTag=v0.5.6`、`approvedChannel=latest`
- release scope：修复 `migrate-runtime` 迁移文本中旧 `aw-set-harness-goal-skill` 引用覆盖不足的问题，并防止 `.servo` scaffold generate 在 preflight 失败前写入 `.gitignore`
- GitHub Release 是 stable release，target commit `06deb1e848b3600e3d766e89dc81accc95e910b8`，publish workflow run `26386813382` 已成功
- npm `servo-installer@0.5.6` 已发布到 `latest` dist-tag，tarball URL：`https://registry.npmjs.org/servo-installer/-/servo-installer-0.5.6.tgz`

注意：`0.5.6` 是 stable release；默认 `servo-installer` 解析到 `latest` 的 `0.5.6`。当前 npm registry 未设置 `next` dist-tag。
