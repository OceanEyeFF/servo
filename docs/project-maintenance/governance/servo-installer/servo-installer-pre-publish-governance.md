...
确认 packlist 包含入口点、payload descriptor、canonical skill payload 与必要 docs，排除状态/缓存/临时证据；root `README.md` 和 governance/testing/usage docs 指向正确选择器。package version、approval lock、selector 或 CLI surface 变化时，由 owning PlanWork 或明确 follow-up 完成 source version docs freshness 检查；不得提前写入尚未发布的 registry fact。
