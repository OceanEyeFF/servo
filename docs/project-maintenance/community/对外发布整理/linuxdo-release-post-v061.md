...
Servo 是一套**基于控制论的 AI coding harness**，主要目的是让它帮你管“AI 写代码的过程”。
...
LLM 本身是高性能，有智力，但是也有缺陷的部件——长工作担心跑偏、遗忘，正常使用担心幻觉，越界操作。LLM的设计——Transformer的Attention逻辑本来就有概率的相关机制在里面，目的就是要让出错的概率降低，让整个工作有更好的容错和冗余。

系统工程和控制论的核心思想是**设计冗余、反馈和恢复路径，让有缺陷的部件组合起来，系统整体可靠**，系统整体能够平稳的运行，能在设计范围内（生命周期|使用场景等）有不错的表现。

Servo 把 AI coding 建模成一个闭环控制系统。目标是让 LLM 在基础的性能之上，依靠工作流程设计实现更好的表现。每一层有观察（Observe）、有验证（Verify）、有裁决（Judge），不达标就回滚或者补额外的工作。
...
我的做法是，对总的任务目标，先讨论并且规划，目的把自己的idea分成多个MileStone，确认MileStone清单写入到 `.servo` 的管理文档中，然后开新的窗口，逐个MileStone做推进工作。
...
Servo 在每轮 Worktrack 或 Milestone 闭环后主动交还控制权——这叫 Handback。 AI 完成了一个受控执行单元，它需要你确认“这个结果对不对”，或者“下一步往哪走”。
使用者给的新Prompt，如果含有有效信息，控制器拿到之后才知道该干什么。
使用者如果不给有效信息，状态机没接收到有效输入，自然不应该有新的动作。

**所以只是使用类似“继续工作”“继续推进”，是不被允许用做推进Servo管理下的工作进度的。**
设计这个工作思路的原因是，担心使用者不做复查不做思考，直接无脑开下一个任务。
有一种情况例外，就是Servo建议你可以进入到下一个环节，或者有其他的工作建议需要你的批准。这个时候使用者的Prompt会被视作权限审批。
...
CodeX的 `/goal` 和 `/plan` 本身与servo并不冲突。
`/goal` 的内容相对来说还比较简单，在说清楚MileStone目标、工作机制不被打断（no handback until milestone finished），的时候，有概率触发 /goal 。
`/plan` 也可以用于规划MileStone|规划Worktrack，然后再把Plan的内容写入到 .servo 路径下的MileStone|Worktrack管理文件中。
也可以使用 `/plan` 来做Worktrack的任务推进管理，虽然我不是很推荐……至少对于GPT是多此一举了。
