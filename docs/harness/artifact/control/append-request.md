...
`append-milestone` 用于表达 milestone 级追加：创建新的 milestone、注册 planned milestone、激活符合条件的 planned milestone，或向已有 milestone 追加 worktrack 的请求。其分类结果优先为 `new milestone`，下一路由为 `milestone-init-skill`，必须输出 milestone brief 所需的最小字段、审批边界和 `recommended_next_scope: RepoScope`。若请求只是向当前活跃 worktrack 扩范围，仍按 `scope expansion` 处理；若改变 Goal Charter，仍按 `goal change` 处理。
