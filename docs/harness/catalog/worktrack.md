...
`WorktrackScope` skills 负责局部状态转移闭环，消费 contract/plan/evidence/control state，可派发下游 `SubAgent` 但不伪装成”控制平面+执行平面一体”。worktrack-schedule-skill 是 `selected_next_action` 与 dispatch handoff packet 的唯一 authority；worktrack-dispatch-skill 只消费 scheduling packet 不反向改写 queue。worktrack-generic-worker-skill 是无专用 skill 时的通用执行载体；worktrack-doc-catch-up-skill 是 Harness 入口观察和 closeout 前推荐使用的文档基线追平载体，release / publish / version / VCS tracking 事实变化后也必须执行版本事实同步。freshly seeded 或 autonomous continuation 的首个 execution-facing round，初始 slice 必须先收紧到最小可验证子片段。`runtime_dispatch_mode` 读取顺序：默认 worktrack-contract-primary 下 contract 的 `runtime_dispatch_mode` 优先；仅 global-override 时 control-state 覆盖；contract 未声明时使用 control-state 默认值。control-state 的 `subagent_dispatch_mode` 与 `subagent_dispatch_mode_override_scope` 只提供 repo 级默认和覆盖边界。`runtime_dispatch_mode` 支持 `auto`/`delegated`/`current-carrier`，默认 `auto`；无法分派需记录 runtime fallback 和 dispatch package unsafe 等边界事实。

Worktrack Contract 的 `node_type` 字段合法值来自 [Node Type Registry](../artifact/control/node-type-registry.md)，Contract 中的 `baseline_form`、`merge_required`、`gate_criteria`、`if_interrupted_strategy` 从 Registry 继承默认值并可在 Contract 中显式覆盖；worktrack-gate-skill 根据 `node_type` 查找对应 `gate_criteria` 确定需要收集的证据面。

Worktrack Contract 还拥有 Branch Policy 字段：`baseline_branch` 表示 servo-managed final baseline；`branch_source_ref` 表示 Worktrack branch 从哪里创建；`integration_target_ref` / `closeout_target_ref` 表示 closeout 直接合入哪里。Milestone-derived Worktrack 默认从 active Milestone branch 创建并合回该 Milestone branch，Milestone final acceptance 后再由 Milestone closeout 合回 baseline branch。下游 init/close/refresh skills 不得从当前 checkout 或默认分支名推断这些目标。
...
### 1. worktrack-init-skill

职责：创建 bounded branch、初始化 baseline 和 Worktrack Contract、建立最小 Plan/Task Queue、产出面向 worktrack-schedule-skill 的 seed/handoff 而非新鲜权威 dispatch packet。
...
- [../../../product/harness/skills/worktrack-init-skill/SKILL.md](../../../product/harness/skills/worktrack-init-skill/SKILL.md)
...
- [../../../product/harness/skills/worktrack-schedule-skill/SKILL.md](../../../product/harness/skills/worktrack-schedule-skill/SKILL.md)
...
### 3. worktrack-dispatch-skill

职责：接收 Worktrack 的下一任务，校验 scheduling handoff packet，拒收 oversized packet 退回 worktrack-schedule-skill。优先选择语义匹配的专门 skill；无专门 skill 时 fallback 到 worktrack-generic-worker-skill 或 current-carrier。文档追平优先绑定 worktrack-doc-catch-up-skill。按 runtime_dispatch_mode 选择载体：auto 按 Dispatch Decision Policy 选择 SubAgent、专用 skill、generic worker 或 current-carrier；delegated 必须分派否则返回 gap/block；current-carrier 显式关闭分派。跑一轮 bounded execution 并回传 evidence。
...
选择规则：只有专门 skill 对当前 work item 有清晰语义贴合时才优先绑定；否则 fallback 到 worktrack-generic-worker-skill 且不得扩大 scope 或绕过 verification_requirements。文档追平优先绑定 worktrack-doc-catch-up-skill。handoff packet 缺失/不完整或已膨胀成多 slices/多队列项时，必须退回 worktrack-schedule-skill。

canonical executable source：

- [../../../product/harness/skills/worktrack-dispatch-skill/SKILL.md](../../../product/harness/skills/worktrack-dispatch-skill/SKILL.md)

当前状态：

- `initial canonical executable skeleton landed`

### 4. worktrack-generic-worker-skill
...
- [../../../product/harness/skills/worktrack-generic-worker-skill/SKILL.md](../../../product/harness/skills/worktrack-generic-worker-skill/SKILL.md)

当前状态：

- `initial canonical executable skeleton landed`

### 5. worktrack-doc-catch-up-skill
...
- [../../../product/harness/skills/worktrack-doc-catch-up-skill/SKILL.md](../../../product/harness/skills/worktrack-doc-catch-up-skill/SKILL.md)
...
### 6. worktrack-review-evidence-skill
...
- [../../../product/harness/skills/worktrack-review-evidence-skill/SKILL.md](../../../product/harness/skills/worktrack-review-evidence-skill/SKILL.md)
...
### 7. worktrack-test-evidence-skill
...
- [../../../product/harness/skills/worktrack-test-evidence-skill/SKILL.md](../../../product/harness/skills/worktrack-test-evidence-skill/SKILL.md)
...
- [../../../product/harness/skills/worktrack-rule-check-skill/SKILL.md](../../../product/harness/skills/worktrack-rule-check-skill/SKILL.md)

当前状态：

- `initial canonical executable skeleton landed`

### 9. worktrack-gate-skill
...
- [../../../product/harness/skills/worktrack-gate-skill/SKILL.md](../../../product/harness/skills/worktrack-gate-skill/SKILL.md)
...
- [../../../product/harness/skills/worktrack-recover-skill/SKILL.md](../../../product/harness/skills/worktrack-recover-skill/SKILL.md)

当前状态：

- `initial canonical executable skeleton landed`

### 11. worktrack-close-skill
...
- [../../../product/harness/skills/worktrack-close-skill/SKILL.md](../../../product/harness/skills/worktrack-close-skill/SKILL.md)

当前状态：

- `initial canonical executable skeleton landed`

### 12. repo-writeback-skill

职责：通用 .servo artifact 写回算子。接收结构化 writeback_instruction，执行文件级事务（预校验→重写→后校验）。替代 harness-skill 和 worktrack-close-skill 中分散的 ad-hoc 写回逻辑。覆盖 milestone artifact / backlog / history / control-state / worktrack-backlog 的写回模式。
...
- [../../../product/harness/skills/repo-writeback-skill/SKILL.md](../../../product/harness/skills/repo-writeback-skill/SKILL.md)
