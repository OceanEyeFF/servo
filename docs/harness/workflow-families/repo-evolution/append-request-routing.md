...
追加请求 intake 发生在执行前，回答"这条新增要求应进入哪条控制路由"。支持 append-feature 和 append-design 两个 mode，由同一个 `repo-append-request-skill` 承接。
...
goal change 优先于所有其他分类。scope expansion 与 new worktrack 冲突时以用户是否要求纳入活跃 worktrack 为判定点。append-design 仅在用户明确授权实现时才归为 design-then-implementation。分类证据不足时返回最小缺失信息。

## 输出

路由结果包含 mode、原始请求、分类结果与理由、目标影响、活跃 worktrack 影响、下一路由与 scope、权限边界、最小缺失信息、continuation readiness。
