...
> 目的：为 operator 提供安全卸载 servo-installer 部署产物的操作指南，明确 `prune --all` 的受管边界、删除范围、保留内容和 bundle 模式行为。
...
| 不确定安装了什么 | 先运行 `servo-installer diagnose` 查看当前状态 |
...
`prune --all` 删除当前 backend 可识别的**所有受管目录**。受管目录是由 servo-installer 在此前 `install` 操作中创建的目录，通过 `aw.marker` 文件识别。
...
- **不卸载** `servo-installer` npm package 本身（用 `npm uninstall -g servo-installer` 处理）
...
```bash
servo-installer prune --all --backend claude
```
...
```bash
servo-installer prune --all --backend bundle
```
...
servo-installer diagnose --backend bundle --json
```
...
彻底移除 servo-installer 的所有痕迹：

| 步骤 | 命令 | 说明 |
|------|------|------|
| 1. 查看当前状态 | `servo-installer diagnose --backend bundle` | 了解受管目录范围 |
...
2. 解决权限或文件锁定问题
3. 对失败的 backend 单独重试 `servo-installer prune --all --backend <failed-backend>`
4. 运行 `servo-installer diagnose` 确认清理完成
