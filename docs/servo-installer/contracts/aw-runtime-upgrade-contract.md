...
- **path reference rewriting：`.aw/` -> `.servo/`，`` `.aw` `` -> `` `.servo` ``，`aw-set-harness-goal-skill` -> `repo-init-goal-skill` in migrated text files；branch names（`develop-aw`、`aw/demo-*`）和 `aw.marker` 不能被改写**
