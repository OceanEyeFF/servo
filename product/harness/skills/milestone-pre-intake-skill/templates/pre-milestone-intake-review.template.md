...
> Use this template when `milestone-pre-intake-skill` needs to organize a before-start question review before `milestone-init-skill` may create, update, or activate a milestone. This template is an output scaffold, not a long-term truth source.
...
This brief is a draft. It is not a milestone artifact until `milestone-init-skill` consumes a confirmed intake review.
