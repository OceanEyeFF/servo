---
name: milestone-pre-intake-skill
...
本技能运行在 `milestone-init-skill` 之前。它接收 programmer 的自然语言需求、已有 repo truth、当前控制状态和最小代码仓库上下文，先把需求整理为可确认的 milestone brief 草案，再识别模糊点、风险点、范围扩张点和需要 programmer 决策的地方。它的输出是结构化 `pre_milestone_intake_review`，供 `milestone-init-skill` 消费；当目标 milestone 已创建或准备激活时，该 review 同时作为 Milestone Review Gate 的输入。
...
- `milestone-init-skill` 准备 create / upsert / activate，但 milestone brief 仍依赖未确认假设。
...
8. 停止并交给 `milestone-init-skill` 或返回 programmer；本技能不得自行写入/激活 milestone。
...
- Candidate milestone brief 必须保持 recommendation / draft 身份。它可以帮助 programmer 决策，但在 programmer confirmation 和 `milestone-init-skill` 写入前，不得成为 live milestone truth、planned milestone、active milestone 或 Worktrack task queue。
