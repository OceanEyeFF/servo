---
name: worktrack-generic-worker-skill
...
如果任务的主要目标是追平长期文档，请优先使用 `worktrack-doc-catch-up-skill`。如果任务包仍然开放、过大或缺少验收标准，应返回调度阶段，而不是让本技能臆造边界。
...
1. 读取任务 Prompt、`shared_fact_pack` 和 `context_budget`，确认范围、验收标准、验证要求和非目标。
