...
- scanner_command: PYTHONDONTWRITEBYTECODE=1 python3 ./scripts/complexity_signal_scanner.py --repo <repo> --json
  ...
  - scanner_command_alternatives: PYTHONDONTWRITEBYTECODE=1 python3 <repo-init-goal-skill-package-root>/scripts/complexity_signal_scanner.py --repo <repo> --json; use the skill package script path when the operator is not running from the package root, so the command does not depend on the user project relative path.
