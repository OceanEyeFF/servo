...
`harness-set-goal-skill/assets/` 是 `harness-set-goal-skill` 自带的 `.servo` 初始化资产包。
...
- 这些资产只服务 `harness-set-goal-skill` 的 `.servo` 初始化流程
...
- 如果目标 repo 也要给 Claude Code 暴露本技能，可在 `generate` 时追加 `--install-claude-skill`，或单独运行 `install-claude-skill` 子命令；目标路径是 `<deploy-path>/.claude/skills/harness-set-goal-skill/`
