---
name: worktrack-rule-check-skill
...
本技能实现 `WorktrackScope.Verify` 算子的 `policy` lane（策略校验面），对应 Harness 控制回路中的**收集证据**阶段。它负责收集规则与治理维度的证据，为 `worktrack-gate-skill` 的 policy-gate 提供输入。它与 `worktrack-review-evidence-skill`（implementation lane）和 `worktrack-test-evidence-skill`（validation lane）共同构成 Verify 的三个正交校验面。
...
- 规则的输出范围只能限定在当前差异表面与治理产物之内；完整代码仓库审计的行为禁止出现在策略证据的输出中。
- 仅当审查证据或测试证据被明确当作输入引用并记录来源时，将其视为规则检查证据才合法；否则必须将其排除在策略证据之外。
- 当被触及路径明显触发了必要同步义务时，唯一合法行为是将其显式暴露在 `必需同步项` 中；隐藏同步义务的行为必须返回 blocked。
- 输出只能分别呈现违规、缺失证据与推迟检查三个独立段落；把三者混成一段模糊摘要的行为禁止出现在策略证据的输出中。
