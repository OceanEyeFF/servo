...
2. 执行 dry-run，输出：
   ...
   - 停止条件命中情况
3. 必须保留的字段组来自 `docs/harness/artifact/control/control-state.md#control-state-compaction-contract`，至少覆盖 current scope/function、active worktrack、active milestone、branch guard、review gate、approval boundary、continuation authority、handback guard、baseline traceability 和 autonomy ledger。
4. 可折叠内容仅限旧 `latest_closed_worktrack_commit`、旧 `verified_at`、旧 handback note、旧 closeout 摘要和非当前路由所需的重复 checkpoint 叙述。
5. 若需要 externalized history，必须由本次 compact 操作生成 compaction history artifact，并记录 source checkpoint、created_at、preserved field summary 和 externalized sections。
6. 不得把 installer-generated backup/update artifacts 当作 history source、模板默认值、清理输入或 `handback_history_ref` 的默认目标。
7. 写入后重新读取 compacted control-state，验证 hydration-critical 字段可解析；验证失败时保留原文件并返回 blocked / recover 建议。
