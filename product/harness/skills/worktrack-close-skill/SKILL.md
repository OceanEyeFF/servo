...
4. **Pre-Closeout Checks**（在确定收尾阶段之前）：
   - **Self-Review**：执行 Self-Review Contract 定义的结构化自查，覆盖 artifact 更新完整性、scope 合规、docs 一致性三个维度。产出 structured self-review record。若 `overall_verdict = blocked`，handback 修复后重试，不进入收尾阶段。
   - **Single-Acceptance**：执行 Single-Acceptance Contract 定义的单体验收，对照 worktrack contract 的 `completion_signals` 逐条验证。产出 structured single-acceptance verdict。`accepted` / `accepted_with_notes` 允许继续；`blocked` 触发 critical failure 升级阻断，需区分以下子路径：
   ...
   - **Post-Feedback Update Revalidation**：若 review feedback、Worktrack Gate feedback、Closeout Gate feedback、Self-Review 或 Single-Acceptance feedback 之后发生 post-review/post-gate updates，Close 不得把更新前的 Gate/review/acceptance evidence 当作 final acceptance evidence。Single-Acceptance 和 Closeout Gate 必须接收更新后的 `updated_files_or_artifacts` 与 `post_update_validation_timestamp`，并确认 validation and acceptance evaluation 已在更新后重跑；否则 closeout 阶段阻塞并回到验证/验收重跑。
   ...
   - closeout evidence 必须保留 `task_producing_subagent` 与 `review_acceptance_subagent` 的 role provenance。`producer_carrier_ref`、`review_carrier_ref`、`acceptance_carrier_ref` 和 `evidence_provenance` 必须分开写入；若收尾只能使用 `current-carrier fallback` 验收，必须记录 `fallback_reason_code`，不得把 task-producing carrier 的自述提升为独立 acceptance。
5. 判断当前收尾阶段：
   - `准备合并请求`
   - `合并请求已开`
   - `准备合并`
...
8. 写回动作（progress_counter、latest_closed_worktrack_commit、control-state route 等）使用 repo-writeback-skill 执行，不再使用 ad-hoc 字段写入。
...
- `closeout_evidence_bundle`
  ...
  - `post_feedback_update_revalidation`
    - `updated_files_or_artifacts`
    - `post_update_validation_timestamp`
    - `validation_after_update_status`
    - `acceptance_after_update_status`
    - `pre_update_final_acceptance_evidence_status`
  - `bundle_completeness`
...
- `closeout_evidence_bundle.post_feedback_update_revalidation` 必须保留 `updated_files_or_artifacts` 与 `post_update_validation_timestamp`。当 `updated_files_or_artifacts` 非 `none` 时，更新前的 final acceptance evidence 必须标记为 stale/history，不得作为 closeout 的验收依据；缺少更新后 validation and acceptance evidence 时必须阻塞 Close。
...
- Close 必须把 `task_producing_subagent` 与 `review_acceptance_subagent` 的 `evidence_provenance` 原样传播到 `closeout_evidence_bundle`。若 `producer_carrier_ref` 与 `review_carrier_ref` / `acceptance_carrier_ref` 相同，只有在 `current-carrier fallback` 且 `fallback_reason_code` 为 `runtime_gap`、`permission_blocked`、`coupling_or_shared_state`、`dispatch_package_unsafe` 或 `not_applicable` 时才能继续；否则 closeout bundle 必须标记为 `contaminated` 或 `incomplete`。
