...
`工作追踪约定` 和 `计划/任务队列` 不是代码仓库级任务来源。它们只能作为关于当前活动中或刚关闭工作追踪的边界证据被查询，例如前一个切片是否完成、继续权限是否受限、或某个交接包是否仍在生效。一个关闭的队列不代表代码仓库没有下一步，只代表那个工作追踪的本地执行序列已经结束。本技能对 `.servo/worktrack/*` 的唯一合法行为是将其读取为边界证据；更新或重写 `.servo/worktrack/*` 的行为必须返回 blocked。
...
12. 执行 Milestone-First 判定：
    若 `active_milestone` 为空（无活跃 Milestone）：
      ...
      f2. 若命中 complex-project trigger，必须同时输出或消费 `complex_project_entry_gate`。`milestone_blocking_decision` 包含 `block_create`、`block_upsert` 或 `block_activate` 时，推荐 `保持并观察` 或 reinforcement documentation / project-understanding Milestone，不得绑定 `init-milestone-skill` 执行被阻断动作。
