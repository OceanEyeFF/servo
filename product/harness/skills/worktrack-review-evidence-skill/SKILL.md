---
name: worktrack-review-evidence-skill
...
本技能实现 `WorktrackScope.Verify` 算子的 `implementation` lane（实现校验面），对应 Harness 控制回路中的**收集证据**阶段。它负责收集代码审查维度的证据，为 `worktrack-gate-skill` 的 implementation-gate 提供输入。它与 `worktrack-test-evidence-skill`（validation lane）和 `worktrack-rule-check-skill`（policy lane）共同构成 Verify 的三个正交校验面。
...
默认选择：docs-only 小修用 `light`；普通 canonical skill/docs/test 修改用 `standard`；dispatch、authority、path governance、adapter/deploy 安全边界用 `risky`；control-state、gate、installer/publish、destructive action 用 `deep`。
