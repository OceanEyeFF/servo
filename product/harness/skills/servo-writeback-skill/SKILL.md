...
它不承担业务逻辑（如 milestone completion 判定、pipeline advancement 计算），只执行经过验证的字段写入。业务逻辑仍由调用方（harness-skill、close-worktrack-skill 等）负责。
...
## 与 servo-cleanup-skill 的边界

| 维度 | .servo-writeback-skill | servo-cleanup-skill |
|------|----------------------|---------------------|
