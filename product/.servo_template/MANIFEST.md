...
| `repo/snapshot-status.md` | `repo-refresh-skill` | `.servo/repo/snapshot-status.md` 模板 | Repo-owned snapshot seed |
| `worktrack/control-state-wt.md` | `harness-skill` | Worktrack control pointer seed | Candidate artifacts are created per Worktrack, not pre-generated here |
...
| `docs/node-type-registry.md` | template-owned reference | 节点类型默认规则 | Package-local seed |
...
- 真正的 canonical source 是写入该文件的 Skill operational contract；template 只提供运行时骨架。
