...
| `test_unified_milestone_init.py` | Milestone Init 准入、确定性文档合同与 single-writer exact-byte persistence 回归 | milestone, admission, contract, exact-byte, roll-forward |
...
- `test_unified_milestone_init.py`：覆盖 LLM-owned discussion-sufficiency admission 与完整 authoring 合同、宽松 Markdown/opaque prose 读取、严格 control/domain invariant、validation 零写入、approval digest/CAS、single-writer exact-byte persistence、`os.replace` commit point、safe repeat/roll-forward、幂等 branch materialization，以及 approved candidate bytes 不被 normalization 或重新序列化
