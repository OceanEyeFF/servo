...
| `closeout_acceptance_gate.py` | closeout 串联 gate（scope/spec/static/cache/test/smoke）；支持 `--profile lightweight`（仅 governance）和 `--profile full`（全部 6 gate，默认） | closeout, gate, cache, smoke, profile |
...
| Profile | Gates | 用途 | 耗时 |
|---------|-------|------|------|
| `lightweight` | scope_gate, spec_gate, static_gate, cache_gate | docs-only、配置修改、小范围 analysis WT | 快 (< 30s) |
| `full`（默认） | scope_gate, spec_gate, static_gate, cache_gate, test_gate, smoke_gate | 默认行为；release、feature、代码修改 WT | 慢 (数分钟) |

`lightweight` 跳过 test_gate（pytest/npm test/tarball smoke）和 smoke_gate（runtime consistency）。release-sensitive 变更必须用 `full`。
