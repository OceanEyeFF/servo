...
- `test_servo_cleanup_control_state_compact.py`：覆盖 `milestone-cleanup-skill` 的 control-state compact dry-run/apply、preserved-field 校验、generated history ref、`.servo/backup(s)` 排除，以及 `product/.servo_template/control-state.md` / `harness-set-goal-skill/assets/control-state.md` 对 compact preserved-field contract 的模板兼容性
