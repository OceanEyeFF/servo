...
function csiCursorTo(row, col) {
	return haveColor ? `\x1b[${row};${col}H` : "";
}
function csiEraseToEnd() {
	return haveColor ? "\x1b[0J" : "";
}
...
function colorGreen(text) {
	return haveColor ? `${SGR_GREEN}${text}${SGR_RESET}` : text;
}
function colorYellow(text) {
	return haveColor ? `${SGR_YELLOW}${text}${SGR_RESET}` : text;
}
function colorRed(text) {
	return haveColor ? `${SGR_RED}${text}${SGR_RESET}` : text;
}
function colorCyan(text) {
	return haveColor ? `${SGR_CYAN}${text}${SGR_RESET}` : text;
}
function colorDim(text) {
	return haveColor ? `${SGR_DIM}${text}${SGR_RESET}` : text;
}
function colorBold(text) {
	return haveColor ? `${SGR_BOLD}${text}${SGR_RESET}` : text;
}
...
function createRunLogger({ logDir, args, targetRepoRoot, tui }) {
	...
	return {
		...
		logPath: join(
			logDir,
			`${timestampForFileName(startedAt)}-${tui ? "tui" : args[0] || "default"}.json`,
		),
...
function pathSafetyPolicyStringList(fieldName) {
	...
	if (
		...
		throw new Error(
			`path safety policy field must be a string array: ${fieldName}`,
		);
	}
	return value;
}
...
function exactSensitiveTargetRepoRoots() {
	return pathSafetyPolicyStringList("exact_sensitive_target_repo_roots").map(
		(path) => resolveExistingOrLexical(path),
	);
}
...
function recursiveSensitiveTargetRepoRoots() {
	const home = process.env.HOME || "";
	const roots = [
		...pathSafetyPolicyStringList("recursive_sensitive_target_repo_roots"),
	];
	if (home) {
		roots.push(
			...pathSafetyPolicyStringList(
...
function validateNotSensitiveRepoRoot(resolved, subject, action) {
	for (const sensitiveRoot of exactSensitiveTargetRepoRoots()) {
		if (resolved === sensitiveRoot) {
			throw new Error(
				`${subject} is protected and cannot be ${action}: ${resolved}`,
			);
		}
	}
	for (const sensitiveRoot of recursiveSensitiveTargetRepoRoots()) {
		if (
			...
			isPathContainedIn(resolved, sensitiveRoot)
		) {
			throw new Error(
				`${subject} is protected and cannot be ${action}: ${resolved}`,
			);
...
function validateTargetRepoRoot(path, sourceRoot) {
	...
	const allowedPrefixes = pathSafetyPolicyStringList(
		"allowed_target_repo_root_prefixes",
	)
	...
	if (
		!uniqueAllowedPrefixes.some(
			(prefix) => resolved === prefix || isPathContainedIn(resolved, prefix),
		)
	) {
		throw new Error(
...
function validateSourceRepoRoot(path) {
	...
	const missingPaths = requiredPaths
		...
		.map((requiredPath) =>
			relative(resolved, requiredPath).split(sep).join("/"),
		);
...
function resolveTuiTargetRepoRoot() {
	return resolveTargetRepoRoot(
		resolveSourceRoot(),
		Boolean(process.env.SERVO_HARNESS_REPO_ROOT),
	);
}
...
function normalizeRelativePath(value, fieldName, skillId, rootDescription) {
	if (typeof value !== "string" || value.trim() === "") {
		throw new Error(
			`${fieldName} must be a non-empty relative path for skill ${skillId}`,
		);
	}
	if (value.includes("\0")) {
		throw new Error(
			`${fieldName} must not contain null bytes for skill ${skillId}: ${value}`,
		);
	}
	const normalized = value.replace(/\\/g, "/");
	if (normalized.startsWith("/") || /^[A-Za-z]:/.test(normalized)) {
		throw new Error(
			`${fieldName} must stay within the ${rootDescription} for skill ${skillId}: ${value}`,
		);
	}
	...
	if (invalidSegment) {
		throw new Error(
			`${fieldName} must not contain '${invalidSegment}' path segments for skill ${skillId}: ${value}`,
		);
	}
...
function payloadTargetMetadata(payload, binding) {
	...
	if (
		typeof targetDirValue !== "string" ||
		typeof targetEntryName !== "string"
	) {
		throw new Error(
			`payload target metadata is invalid for skill ${binding.skillId}`,
		);
	}
	if (requiredPayloadFiles === null) {
		throw new Error(
			`payload required_payload_files must be a string array for skill ${binding.skillId}`,
		);
	}
	...
	if (targetDir.includes("/")) {
		throw new Error(
			`payload target_dir must be a single directory name for skill ${binding.skillId}: ${targetDirValue}`,
		);
	}
	...
	const requiredFiles = requiredPayloadFiles.map((entry) =>
		normalizeRelativePath(
			entry,
			"payload required_payload_files entry",
			binding.skillId,
			"backend target root",
		),
	);
	...
	if (!requiredFiles.includes(payloadDescriptor)) {
		throw new Error(
			`payload required_payload_files must include ${payloadDescriptor} for skill ${binding.skillId}`,
		);
	}
	if (!requiredFiles.includes(managedSkillMarker)) {
		throw new Error(
			`payload required_payload_files must include ${managedSkillMarker} for skill ${binding.skillId}`,
		);
	}

	const legacyTargetDirsRaw =
		payload.legacy_target_dirs === undefined
			? []
			: stringList(payload.legacy_target_dirs);
	if (legacyTargetDirsRaw === null) {
		throw new Error(
			`payload legacy_target_dirs must be a list of strings for skill ${binding.skillId}`,
		);
	}
	const legacyTargetDirs = legacyTargetDirsRaw.map((entry) =>
		normalizeRelativePath(
			entry,
			"payload legacy_target_dirs entry",
			binding.skillId,
			"backend target root",
		),
	);
	...
	if (legacyTargetDirs.includes(targetDir)) {
		throw new Error(
			`payload target_dir ${targetDir} must not be listed in legacy_target_dirs for skill ${binding.skillId}`,
		);
	}

	const legacySkillIdsRaw =
		payload.legacy_skill_ids === undefined
			? []
			: stringList(payload.legacy_skill_ids);
	if (legacySkillIdsRaw === null) {
		throw new Error(
			`payload legacy_skill_ids must be a list of strings for skill ${binding.skillId}`,
		);
	}
	const legacySkillIds = legacySkillIdsRaw.map((entry) =>
		normalizeRelativePath(
			entry,
			"payload legacy_skill_ids entry",
			binding.skillId,
			"backend target root",
		),
	);
	...
	if (legacySkillIds.includes(binding.skillId)) {
		throw new Error(
			`binding skill_id ${binding.skillId} must not be listed in legacy_skill_ids for skill ${binding.skillId}`,
		);
	}
...
function loadBindingPayloads(bindings) {
	...
	for (const binding of bindings) {
		try {
			loadedPayloads.set(
				binding.payloadPath,
				bindingPayloadWithText(binding, null),
			);
		} catch (error) {
	...
	if (stat === null) {
		return [
			issue(
				"missing-target-root",
				targetRoot,
				`${backend} target root does not exist`,
			),
		];
	}
		...
		if (existsSync(targetRoot)) {
			return [
				issue(
					...
					targetRoot,
					"target root must be a real directory, not a symlink",
				),
			];
		}
		return [
			issue(
				"broken-target-root-symlink",
				targetRoot,
...
function validateGithubRepo(value) {
	if (!githubRepoPattern.test(value)) {
		throw new Error(
			`GitHub repo must use OWNER/REPO with safe characters only: ${value}`,
		);
	}
	return value;
}
...
function validateSha256Digest(value) {
	if (!sha256HexPattern.test(value)) {
		throw new Error(
			`SHA256 digest must be 64 hexadecimal characters: ${value}`,
		);
	}
...
function defaultGithubSourceRepo() {
	return (
		process.env.SERVO_INSTALLER_GITHUB_REPO ||
...
function findZipEndOfCentralDirectory(buffer) {
	...
	for (
		let offset = buffer.length - zipEocdMinBytes;
		offset >= minOffset;
		offset -= 1
	) {
...
function zipEntries(buffer) {
	...
	const centralDirOffset = buffer.readUInt32LE(
		eocdOffset + zipEocdCentralDirectoryOffset,
	);
	const entries = [];
	...
	for (let index = 0; index < entryCount; index += 1) {
		...
		const method = buffer.readUInt16LE(
			offset + zipCentralDirectoryMethodOffset,
		);
		const crc32Value = buffer.readUInt32LE(
			offset + zipCentralDirectoryCrc32Offset,
		);
		const compressedSize = buffer.readUInt32LE(
			offset + zipCentralDirectoryCompressedSizeOffset,
		);
		const uncompressedSize = buffer.readUInt32LE(
			offset + zipCentralDirectoryUncompressedSizeOffset,
		);
		const fileNameLength = buffer.readUInt16LE(
			offset + zipCentralDirectoryFileNameLengthOffset,
		);
		const extraLength = buffer.readUInt16LE(
			offset + zipCentralDirectoryExtraLengthOffset,
		);
		const commentLength = buffer.readUInt16LE(
			offset + zipCentralDirectoryCommentLengthOffset,
		);
		const localHeaderOffset = buffer.readUInt32LE(
			offset + zipCentralDirectoryLocalHeaderOffset,
		);
		if (
			...
			throw new Error(
				"GitHub source archive ZIP64 entries are not supported by the Node path",
			);
		}
...
function githubArchiveUncompressedLimitError(maxBytes, entryName = undefined) {
	...
	return new Error(
		`GitHub source archive uncompressed size exceeds ${maxBytes} byte limit${suffix}`,
	);
}
...
function zipEntryData(buffer, entry, options = {}) {
	...
	if (localOffset + zipLocalFileHeaderBytes > buffer.length) {
		throw new Error(
			`GitHub source archive local header is invalid: ${entry.name}`,
		);
	}
	...
	const fileNameLength = buffer.readUInt16LE(
		localOffset + zipLocalFileNameLengthOffset,
	);
	const extraLength = buffer.readUInt16LE(
		localOffset + zipLocalExtraLengthOffset,
	);
	const dataStart =
		localOffset + zipLocalFileHeaderBytes + fileNameLength + extraLength;
	const dataEnd = dataStart + entry.compressedSize;
	if (dataEnd > buffer.length) {
		throw new Error(
			`GitHub source archive entry data is invalid: ${entry.name}`,
		);
	}
	...
	if (entry.method === 0) {
		...
		try {
			data = inflateRawSync(
				...
				maxUncompressedBytes === undefined
					? undefined
					: { maxOutputLength: maxUncompressedBytes + 1 },
			);
		} catch (error) {
			if (maxUncompressedBytes !== undefined) {
				throw githubArchiveUncompressedLimitError(
		...
		throw new Error(
			`GitHub source archive uses unsupported ZIP compression method ${entry.method}: ${entry.name}`,
		);
	}
	if (
		maxUncompressedBytes !== undefined &&
		data.length > maxUncompressedBytes
	) {
	...
	if (crc32(data) !== entry.crc32) {
		throw new Error(
			`GitHub source archive entry CRC32 mismatch: ${entry.name}`,
		);
	}
	return data;
}
...
function safeExtractZipBuffer(buffer, destination, options = {}) {
	const maxUncompressedBytes =
		options.maxUncompressedBytes === undefined
			? githubArchiveMaxUncompressedBytes
			: options.maxUncompressedBytes;
	mkdirSync(destination, { recursive: true });
	if (readdirSync(destination).length > 0) {
		throw new Error(
			`GitHub archive extraction destination must be empty: ${destination}`,
		);
	}
	const stagingRoot = mkdtempSync(
		join(dirname(destination), "servo-installer-extract-"),
	);
	try {
		...
		for (const entry of zipEntries(buffer)) {
			...
			const data = zipEntryData(buffer, entry, {
				maxUncompressedBytes: remainingBytes,
			});
...
function githubSourceRootFromArchiveBuffer(
	repo,
	ref,
	archiveBuffer,
	...
	if (archiveSha256 !== undefined) {
		...
		if (actualSha256 !== expectedSha256) {
			throw new Error(
				`GitHub source archive SHA256 mismatch: expected ${expectedSha256}, got ${actualSha256}`,
			);
		}
	}
	const tempRoot = mkdtempSync(
		join(tmpdir(), "servo-installer-github-source-"),
	);
	try {
		...
		const sourceRoot = validateSourceRepoRoot(
			extractedArchiveRoot(extractRoot),
		);
		return {
			sourceRoot,
...
function githubArchiveDownloadError(repo, ref, message, retryable = false) {
	const error = new Error(
		`Failed to download GitHub source archive ${repo}@${ref}: ${message}`,
	);
	...
	return new Promise((resolvePromise, rejectPromise) => {
		...
		const request = https.get(
			...
			(response) => {
				response.on("error", (error) => {
					if (
						...
						error.message.startsWith(
							"Failed to download GitHub source archive ",
						)
					) {
						fail(error);
						return;
				...
				if (
					...
					const error = githubArchiveDownloadError(
						repo,
						ref,
						`archive exceeds ${options.maxBytes} byte limit`,
					);
					fail(error);
		...
		request.on("error", (error) => {
			if (
				error &&
				error.retryable !== undefined &&
				error.message.startsWith("Failed to download GitHub source archive ")
			) {
				fail(error);
				return;
			}
...
async function downloadGithubArchive(repo, ref, options = {}) {
	...
	const safeRef = validateGithubRef(ref);
	const maxBytes =
		options.maxBytes === undefined ? githubArchiveMaxBytes : options.maxBytes;
	const maxAttempts =
		options.maxAttempts === undefined
			? githubArchiveMaxAttempts
			: options.maxAttempts;
	const retryDelayMs =
		options.retryDelayMs === undefined ? 250 : options.retryDelayMs;
	const timeoutMs =
		options.timeoutMs === undefined ? 60_000 : options.timeoutMs;
	if (!Number.isSafeInteger(maxBytes) || maxBytes <= 0) {
		throw new Error("GitHub archive maxBytes must be a positive safe integer");
	}
	if (!Number.isSafeInteger(maxAttempts) || maxAttempts <= 0) {
		throw new Error(
			"GitHub archive maxAttempts must be a positive safe integer",
		);
	}
	let lastError = null;
	for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
		try {
			return await downloadGithubArchiveAttempt(safeRepo, safeRef, {
				maxBytes,
				timeoutMs,
			});
		} catch (error) {
...
function canonicalSourceMetadata(payload, binding, context) {
	...
	if (typeof canonicalDirValue !== "string" || canonicalPaths === null) {
		throw new Error(
			`payload canonical_dir and canonical_paths must be defined for skill ${binding.skillId}`,
		);
	}
...
function verifySourceBinding(binding, context, loadedPayloads = null) {
	...
	try {
		...
		return [
			issue("payload-contract-invalid", binding.payloadPath, error.message),
		];
	}
	...
	const canonicalDir = join(
		...
		canonicalSource === null
			? String(payload.canonical_dir || "")
			: canonicalSource.canonicalDir,
	);
	if (!isDirectory(canonicalDir)) {
		issues.push(
			issue(
				...
				canonicalDir,
				`missing canonical directory for skill ${binding.skillId}`,
			),
		);
	}
	if (
		canonicalSource !== null &&
		canonicalSource.canonicalDir.split("/").at(-1) !== binding.skillId
	) {
		issues.push(
			issue(
	...
	if (canonicalSource !== null) {
		for (const [
			includedPath,
			canonicalFile,
		] of canonicalSource.canonicalFiles) {
			if (!isFile(canonicalFile)) {
				issues.push(
					issue(
						"missing-canonical-source",
						canonicalFile,
						`missing canonical file ${includedPath} for skill ${binding.skillId}`,
					),
				);
			}
		}
	...
	if (payload.skill_id !== binding.skillId) {
		issues.push(
			issue(
				"payload-contract-invalid",
				binding.payloadPath,
				`payload skill_id must be ${binding.skillId}`,
			),
		);
	}
	...
	try {
		...
		if (
			JSON.stringify(targetMetadata.requiredPayloadFiles) !==
			JSON.stringify(expectedRequiredFiles)
		) {
			issues.push(
				issue(
					"payload-contract-invalid",
...
function sourcePathForTargetRelativeFile(
	binding,
	relativeName,
	context,
	payload,
	...
	if (relativeName === managedSkillMarker) {
		throw new Error(
			`${managedSkillMarker} is runtime-generated for skill ${binding.skillId}`,
		);
	}
...
function claudeFrontmatterOverrides(payload, binding) {
	...
	if (
		payload.claude_frontmatter === null ||
		Array.isArray(payload.claude_frontmatter) ||
		typeof payload.claude_frontmatter !== "object"
	) {
		throw new Error(
			`payload claude_frontmatter must be an object for skill ${binding.skillId}`,
		);
	}
	const overrides = {};
	for (const [key, value] of Object.entries(payload.claude_frontmatter)) {
		if (key === "") {
			throw new Error(
				`payload claude_frontmatter keys must be non-empty strings for skill ${binding.skillId}`,
			);
		}
		if (typeof value !== "boolean") {
			throw new Error(
				`payload claude_frontmatter values must be booleans for skill ${binding.skillId}`,
			);
		}
		overrides[key] = value;
	}
			...
			for (const line of lines.slice(1, closingIndex)) {
				...
				if (Object.hasOwn(overrides, key)) {
					updatedFrontmatter.push(
						`${key}: ${renderFrontmatterValue(overrides[key])}\n`,
					);
					seenKeys.add(key);
				} else {
...
function targetFrontmatterOverrides(binding, relativeName, metadata, payload) {
	if (
		relativeName !== metadata.targetEntryName ||
		binding.backend !== claudeBackend
	) {
		return {};
	}
...
function computePayloadFingerprint(
	...
	if (typeof payload.payload_version !== "string") {
		throw new Error(
			`payload payload_version must be a string for skill ${binding.skillId}`,
		);
	}
	...
	for (const relativeName of metadata.requiredPayloadFiles) {
		if (
			relativeName === managedSkillMarker ||
		...
		try {
			...
			if (error.code === "ENOENT") {
				throw new Error(
					`Missing payload source file while computing fingerprint: ${sourcePath}`,
				);
			}
			throw error;
		}
	...
	return createHash("sha256")
		.update(fingerprintParts.join(""), "utf8")
		.digest("hex");
}
...
function targetRootChildren(targetRoot, action = "verify target root") {
	try {
		...
		throw new Error(
			`Failed to scan ${action} at ${targetRoot}: ${error.message}`,
		);
	}
}
...
function targetRootIdentity(path) {
	...
	if (stat.isSymbolicLink()) {
		if (existsSync(path)) {
			throw new Error(
				`Target root must be a real directory, not a symlink: ${path}`,
			);
		}
...
function assertDirectoryIdentityCurrent(identity, action) {
	...
	try {
		...
		if (error.code === "ENOENT") {
			throw new Error(
				`Target root changed during ${action}, refusing to continue: ${identity.path}`,
			);
		}
		throw error;
	}
...
function ensureInstallTargetRoot(path) {
	...
	try {
		...
		throw new Error(
			`Target root could not be created: ${path}: ${error.message}`,
		);
	}
	const identity = targetRootIdentity(path);
...
function collectTargetDirMetadata(bindings, loadedPayloads = null) {
	...
	for (const binding of bindings) {
		const metadata = payloadTargetMetadata(
			bindingPayloadObject(binding, loadedPayloads),
			binding,
		);
		if (targetDirs.has(metadata.targetDir)) {
			throw new Error(
				`Multiple skills map to the same target_dir for backend ${binding.backend}: ${metadata.targetDir}`,
			);
		}
...
function collectAllKnownTargetDirs(bindings, loadedPayloads = null) {
	const { metadataByPayloadPath } = collectTargetDirMetadata(
		bindings,
		loadedPayloads,
	);
...
function verifyDeployedMarker(
	...
	if (
		...
		marker.payload_version !== payload.payload_version
	) {
		return {
			fatalIssues: [
				issue(
...
function verifyDeployedPayloadFiles(
	binding,
	targetSkillDir,
	context,
	payload,
	...
	for (const relativeName of metadata.requiredPayloadFiles) {
		...
		if (relativeName === managedSkillMarker) {
			...
			try {
				...
				issues.push(
					targetPayloadReadIssue(error, targetPath, relativeName, binding),
				);
				continue;
			}
			if (
				markerState.markerMatchesSource &&
				markerText !== runtimeMarkerText(markerState.expectedMarker)
			) {
				issues.push(
					issue(
						"target-payload-drift",
		...
		try {
			...
			sourcePath =
				relativeName === payloadDescriptor
					? binding.payloadPath
					: sourcePathForTargetRelativeFile(
							binding,
							relativeName,
							context,
							payload,
		...
		let targetText;
		try {
			sourceText = expectedTargetFileText(
				binding,
				relativeName,
				context,
				payload,
...
function verifyDeployedSkill(
	binding,
	targetRoot,
	context,
	loadedPayloads = null,
) {
	const state = loadDeployedSkillState(
		binding,
		targetRoot,
		context,
		loadedPayloads,
	...
	const directoryIssues = verifyDeployedSkillDirectory(
		binding,
...
function unexpectedManagedTargetDirs(
	targetRoot,
	expectedTargetDirNames,
	targetChildren,
...
function verifyBackend(context, options = {}) {
	...
	const collectTargetChildrenOnIssue =
		options.collectTargetChildrenOnIssue === true;
	...
	let children = null;
	if (
		(issues.length === 0 || collectTargetChildrenOnIssue) &&
	...
	if (issues.length === 0 && children !== null) {
		for (const binding of bindings) {
			issues.push(
				...verifyDeployedSkill(binding, targetRoot, context, loadedPayloads),
			);
		}
		issues.push(
			...unexpectedManagedTargetDirs(
				targetRoot,
				expectedTargetDirNames,
				children,
				backend,
...
function managedInstallDirs(
	...
	const children =
		targetChildren === null ? targetRootChildren(targetRoot) : targetChildren;
...
function targetRootReadyIssuesForAction(targetRoot, backend = agentsBackend) {
	return verifyTargetRoot(targetRoot, backend).filter(
...
function assertManagedDirectoryIdentityCurrent(identity) {
	...
	if (
		...
		throw new Error(
			`Managed skill dir changed during pruning, refusing to remove: ${identity.path}`,
		);
	}
	return true;
}
...
function pruneBackendManagedInstalls(context) {
	const targetRootIssues = targetRootReadyIssuesForAction(
		context.targetRoot,
		context.backend,
	);
	if (targetRootIssues.length > 0) {
		throw new Error(
			targetRootReadyFailureMessage("prune managed installs", targetRootIssues),
		);
	}
	...
	let removedCount = 0;
	for (const child of targetRootChildren(
		context.targetRoot,
		"managed install pruning",
	)) {
		...
		try {
			...
			throw new Error(
				`Failed to remove managed skill dir ${child}: ${error.message}`,
			);
		}
		removedCount += 1;
 ...
 */
function collectUpdateTargetEntryIssues(
	targetRoot,
	knownTargetDirNames,
	targetChildren,
	backend = agentsBackend,
	...
	for (const plan of plans) {
		for (const legacyDirName of plan.targetMetadata.legacyTargetDirs) {
			...
			const marker =
				identity === null
					? null
					: loadRuntimeMarker(join(legacyPath, managedSkillMarker));
			if (
				marker !== null &&
...
function formatPathConflicts(conflicts) {
	return [
		...
		...conflicts.map(
			(conflict) =>
				`- ${conflict.skillId}: ${conflict.path} (${conflict.detail})`,
		),
	].join("\n");
}
...
function sourceValidationFailureMessage(action, issues) {
	const details = issues
		.map(
			(currentIssue) =>
				`  - ${currentIssue.code}: ${currentIssue.path} (${currentIssue.detail})`,
		)
		.join("\n");
...
function checkPathsExistSummary(context) {
	const backend = context.backend || agentsBackend;
	const { bindings, loadedPayloads } =
		collectValidatedBindingsForCheckPaths(context);
	const targetRootIssues = verifyTargetRoot(context.targetRoot, backend).filter(
		(currentIssue) => currentIssue.code !== "missing-target-root",
	);
	if (targetRootIssues.length > 0) {
		throw new Error(
			targetRootReadyFailureMessage("check target paths", targetRootIssues),
		);
	}
	...
	const plans = bindings.map((binding) =>
		buildInstallPlan(binding, context.targetRoot, context, {
			loadedPayloads,
			targetMetadata: targetMetadata.metadataByPayloadPath.get(
				binding.payloadPath,
			),
		}),
	);
...
function sourceTextForTargetRelativeFile(
	binding,
	relativeName,
	context,
	payload,
...
function installBackendPayloads(context) {
	const { bindings, loadedPayloads } = collectValidatedBindingsForAction(
		context,
		"install",
	);
	const targetRootIssues = verifyTargetRoot(
		context.targetRoot,
		context.backend,
	).filter((currentIssue) => currentIssue.code !== "missing-target-root");
	if (targetRootIssues.length > 0) {
	...
	const targetRootIdentitySnapshot = ensureInstallTargetRoot(
		context.targetRoot,
	);
	for (const plan of plans) {
		...
		for (const legacyDirName of plan.targetMetadata.legacyTargetDirs) {
			...
			if (
				...
				try {
					...
					throw new Error(
						`Failed to remove legacy skill dir ${legacyPath}: ${error.message}`,
					);
				}
				console.log(
					`removed legacy skill dir ${binding.skillId} -> ${legacyPath}`,
				);
			}
		}
	}
	for (const plan of plans) {
		...
		for (const relativeName of targetMetadataForPlan.requiredPayloadFiles) {
			...
			writeDeployedTextFile(
				targetPath,
				sourceTextForTargetRelativeFile(
					binding,
					relativeName,
					context,
					payload,
...
function isUpdateBlockingIssue(currentIssue, managedDeletePaths) {
	...
	if (
		currentIssue.code === "unrecognized-target-directory" &&
		managedDeletePaths.has(currentIssue.path)
	) {
		return false;
	}
	return true;
...
function updatePlanSummary(context) {
	...
	if (bindings.length === 0) {
		...
		try {
			const targetMetadata = collectTargetDirMetadata(bindings, loadedPayloads);
			knownTargetDirNames = knownTargetDirsFromMetadata(
				bindings,
				targetMetadata.metadataByPayloadPath,
			);
	...
	const managedInstallsToDelete = managedInstallDirs(
		targetRoot,
		targetChildren,
		backend,
	);
	...
	const blockingIssues = allIssues.filter((currentIssue) =>
		isUpdateBlockingIssue(currentIssue, managedDeletePaths),
	);
	...
	return {
		...
		...legacyMigration,
		upgrade_guidance:
			legacyMigration.legacy_target_dir_count > 0
...
function diagnosticSummary(result) {
	...
	const managedDirs = managedInstallDirs(
		result.targetRoot,
		result.targetChildren,
		backend,
	);
	let legacyMigration = {
		legacy_target_dir_count: 0,
	...
	if (result.bindings.length > 0 && isDirectory(result.targetRoot)) {
		try {
			...
			const targetMetadata = collectTargetDirMetadata(
				result.bindings,
				loadedPayloads,
			);
			...
			legacyMigration = legacyTargetDirMigrationSummary(
				plans,
				result.targetRoot,
			);
		} catch (_) {
	...
	const issueCodes = [
		...new Set(result.issues.map((currentIssue) => currentIssue.code)),
	].sort();
	const unrecognized = result.issues.filter((currentIssue) =>
		unrecognizedIssueCodes.has(currentIssue.code),
	);
	const conflicts = result.issues.filter((currentIssue) =>
		conflictIssueCodes.has(currentIssue.code),
	);
	return {
		backend,
...
function parsedBackendRoots(backend, agentsRoot, claudeRoot) {
	...
	return {
		backend,
		agentsRoot,
		...(backend === claudeBackend && claudeRoot !== undefined
			? { claudeRoot }
			: {}),
	};
}
...
function parsedGithubOptions(githubRepo, githubRef, githubArchiveSha256) {
	return {
		githubRepo:
			githubRepo === undefined ? defaultGithubSourceRepo() : githubRepo,
...
function parseNodeMigrateRuntimeArgs(args) {
	...
	if (
		parsed.from !== "aw" ||
		parsed.to !== "servo" ||
		(parsed.json && parsed.yes)
	) {
		return null;
	}
	if (
		!backendAllowed(parsed.backend, [
			agentsBackend,
			claudeBackend,
			bundleBackend,
		])
...
function parseNodeUpdateArgs(args) {
	...
	for (let index = 1; index < args.length; index += 1) {
		...
		const githubArchiveSha256Value = readEqualsOption(
			arg,
			cliFlags.githubArchiveSha256,
		);
...
function parsedNodeUpdateResult(parsed, includeYes = false) {
	...
	return {
		...
		...(includeYes ? { yes: true } : {}),
		agentsRoot: parsed.agentsRoot,
		...(parsed.claudeRoot === undefined
			? {}
			: { claudeRoot: parsed.claudeRoot }),
	};
}

function parseNodeBackendRootArgs(
	args,
	command,
	allowedBackends = [agentsBackend, bundleBackend],
) {
	if (args[0] !== command) {
		return null;
	}
...
function parseNodeDiagnoseJsonArgs(args) {
	...
	return parseNodeBackendRootArgs(withoutJson, "diagnose", [
		agentsBackend,
		claudeBackend,
		bundleBackend,
	]);
...
function parseNodeUnsupportedUpdateJsonYesArgs(args) {
	...
	for (let index = 1; index < args.length; index += 1) {
		...
		if (
			arg === cliFlags.githubRepo ||
			arg === cliFlags.githubRef ||
			arg === cliFlags.githubArchiveSha256
		) {
	...
	if (
		...
		!hasYes ||
		backend !== agentsBackend ||
		!backendAllowed(source, [packageSource, githubSource])
	) {
		return null;
	}
...
function runNodeDiagnose(args) {
	...
	try {
		printDiagnosticSummary(
			diagnosticSummary(verifyBackend(buildNodeBackendContext(parsed))),
		);
		return 0;
	} catch (error) {
...
function runDeployServoMigrate(context) {
	validateNotSensitiveRepoRoot(
		context.targetRepoRoot,
		"reconcile-servo target repo root",
		"reconciled",
	);
	const helperArgs = [
		context.helperPath,
...
function runtimeMigrationReinstallPlan(context) {
	...
	let summaries;
	if (context.backend === bundleBackend) {
		const { agentsContext, claudeContext } =
			validateBundleDisjointRoots(context);
	...
	const blockingIssueCount = Object.values(summaries).reduce(
		(total, summary) => total + summary.blocking_issue_count,
		0,
	);
	return {
		requested: true,
	...
	while (stack.length > 0) {
		...
		for (const entry of readdirSync(currentPath, { withFileTypes: true })) {
			...
			try {
				...
				throw new Error(
					`runtime migration blocked: symlink target is unreadable: ${entryPath}`,
				);
			}
...
function copyRuntimeDirectory(sourcePath, destinationPath) {
	...
	throw new Error(
		`runtime migration blocked: unsupported runtime entry type: ${sourcePath}`,
	);
}
...
function runtimeMigrationSummary(context) {
	validateNotSensitiveRepoRoot(
		context.targetRepoRoot,
		"Runtime migration target repo root",
		"migrated",
	);
	...
	return {
		...
		action,
		planned_actions: mutationAllowed
			? [
					"copy .aw to .servo",
					"rewrite .aw path references to .servo in migrated text files",
				]
			: [],
...
function applyRuntimeMigration(context) {
	...
	if (summary.issue_count > 0) {
		throw new Error(
			`runtime migration blocked by ${summary.issue_count} issue(s)`,
		);
	}
	...
	const rewrittenCount = rewriteAwPathsInDirectory(
		context.destinationRuntimePath,
	);
	writeFileSync(
...
function printRuntimeMigrationSummary(summary) {
	console.log(
		`runtime migration ${summary.from} -> ${summary.to} for ${summary.target_repo_root}`,
	);
	...
	if (summary.reinstall_plan.requested) {
		...
		console.log(
			`reinstall blocking issues: ${summary.reinstall_plan.blocking_issue_count}`,
		);
	}
	if (
		summary.rewritten_file_count !== undefined &&
		summary.rewritten_file_count > 0
	) {
...
function githubSourceRecoveryArgs(parsed) {
	...
	if (parsed.githubArchiveSha256 !== undefined) {
		args.push(
			`${cliFlags.githubArchiveSha256} ${JSON.stringify(parsed.githubArchiveSha256)}`,
		);
	}
	return args.join(" ");
}
	...
	try {
		return await withNodeUpdateContext(parsed, (context) => {
			...
			if (summary.blocking_issue_count > 0) {
				throw new Error(
					`[${summary.backend}] update blocked by ${summary.blocking_issue_count} preflight issue(s)`,
				);
			}
			console.log(
				`[${summary.backend}] dry-run only; pass --yes to apply update`,
			);
			return 0;
		});
	} catch (error) {
...
function applyUpdateContext(context) {
	...
	if (summary.blocking_issue_count > 0) {
		throw new Error(
			`[${context.backend}] update blocked by ${summary.blocking_issue_count} preflight issue(s)`,
		);
	}
	...
	try {
		...
		if (result.issues.length > 0) {
			...
			throw new Error(
				`[${context.backend}] update failed strict verify with ${result.issues.length} issue(s)`,
			);
		}
	} catch (error) {
...
function printUpdatePlan(summary) {
	...
	console.log(
		`managed installs to delete: ${summary.managed_installs_to_delete.length}`,
	);
	...
	if (summary.legacy_target_dir_count > 0) {
		console.log(
			`legacy target dirs to replace: ${summary.legacy_target_dir_count}`,
		);
...
function checkBackendTargetPaths(context) {
	...
	console.log(
		`[${context.backend}] ok: no pre-existing paths at ${summary.targetRoot}`,
	);
}
...
function runNodeCheckPathsExist(args) {
	...
	try {
		...
		console.log(
			`[${summary.backend}] ok: no pre-existing paths at ${summary.targetRoot}`,
		);
		return 0;
	} catch (error) {
		console.error(`error: ${error.message}`);
...
function parseNodePruneArgs(args) {
	...
	if (
		!hasAll ||
		!backendAllowed(backend, [agentsBackend, claudeBackend, bundleBackend])
	) {
		return null;
	}
	return {
...
function runNodeUnsupportedAgentsVariant(args) {
	if (parseNodeUnsupportedUpdateJsonYesArgs(args) !== null) {
		console.error(
			"error: update --json is only supported for dry-run plans; omit --json with --yes",
		);
		return 1;
	}
...
function printVerifyResult(result) {
	if (result.issues.length === 0) {
		console.log(
			`[${result.backend}] ok: target root is ready at ${result.targetRoot}`,
		);
		return;
	}
	console.log(
...
function validateBundleDisjointRoots(parsed) {
	const agentsContext = buildNodeBackendContext(
		buildBundleBackendOptions(parsed, agentsBackend),
	);
	const claudeContext = buildNodeBackendContext(
		buildBundleBackendOptions(parsed, claudeBackend),
	);
	const rootsOverlap =
...
function printBundlePartialCompletion(command, agentsStatus, claudeStatus) {
	...
	console.error(
		`[${bundleBackend}] partial ${command}: agents=${agentsResult}, claude=${claudeResult}`,
	);
}
...
async function runBundleDiagnoseJson(parsed) {
	...
	const bundleSummary = {
		...
		total_managed:
			agentsSummary.managed_install_count + claudeSummary.managed_install_count,
	};
...
async function runBundleDiagnose(parsed) {
	const { agentsContext, claudeContext } = validateBundleDisjointRoots(parsed);
	printDiagnosticSummary({
		...diagnosticSummary(verifyBackend(agentsContext)),
		backend: agentsBackend,
	});
	printDiagnosticSummary({
		...diagnosticSummary(verifyBackend(claudeContext)),
...
async function runBundleVerify(parsed) {
	...
	return agentsResult.issues.length > 0 || claudeResult.issues.length > 0
		? 1
		: 0;
}
...
async function runBundleCheckPathsExist(parsed) {
	...
	const agentsArgs = buildBundleArgs(
		"check_paths_exist",
		agentsBackend,
		parsed,
	);
	const claudeArgs = buildBundleArgs(
		"check_paths_exist",
		claudeBackend,
		parsed,
	);
...
async function runBundleInstall(parsed) {
	...
	const agentsCheckArgs = buildBundleArgs(
		"check_paths_exist",
		agentsBackend,
		parsed,
	);
	const claudeCheckArgs = buildBundleArgs(
		"check_paths_exist",
		claudeBackend,
		parsed,
	);
	...
	if (agentsCheckStatus !== 0 || claudeCheckStatus !== 0) {
		console.error(
			`[${bundleBackend}] pre-write check failed; aborting install`,
		);
		return 1;
	}
	const agentsStatus = await runNodeOwned(agentsArgs);
	...
	if (agentsStatus !== 0) {
		console.error(
			`[${bundleBackend}] recovery: run \`servo-installer prune --backend agents --all\` then \`servo-installer install --backend agents\` then \`servo-installer verify --backend agents\``,
		);
	}
	if (claudeStatus !== 0) {
		console.error(
			`[${bundleBackend}] recovery: run \`servo-installer prune --backend claude --all\` then \`servo-installer install --backend claude\` then \`servo-installer verify --backend claude\``,
		);
	}
	return 1;
}
...
async function runBundleUpdateYes(parsed) {
	...
	if (
		agentsSummary.blocking_issue_count > 0 ||
		claudeSummary.blocking_issue_count > 0
	) {
		console.error(
			`[${bundleBackend}] pre-write update preflight failed; aborting update`,
		);
		return 1;
	}
	const agentsArgs = buildBundleArgs("update", agentsBackend, parsed);
...
async function runBundlePrune(parsed) {
	...
	if (agentsStatus !== 0) {
		...
		console.error(
			`[${bundleBackend}] partial prune: agents failed, claude not started`,
		);
		return 1;
	}
	const claudeStatus = await runNodeOwned(claudeArgs);
...
async function runNodeOwned(args) {
	...
	console.error(
		"Run servo-installer --help for supported package/runtime commands.",
	);
	return 1;
}
...
function buildDiagnoseSummary(capturedOutput, backend) {
	...
	if (!diagnosis || !diagnosis.backends) {
		return {
			ok: false,
			summary: `${SYM_FAIL} Could not parse diagnose output.`,
			backends: [],
		};
	}
	...
	const targetBackends =
		backend === bundleBackend ? [agentsBackend, claudeBackend] : [backend];
...
function checkAwHealth() {
	...
	if (!fs.existsSync(awDir)) {
		return [
			{
				item: ".servo directory",
				ok: false,
				detail: "missing - Harness not initialized",
			},
		];
	}
	...
	if (fs.existsSync(msDir)) {
		try {
			const files = fs
				.readdirSync(msDir)
				.filter((f) => f.endsWith(".md") && f !== "milestone-template.md");
			checks.push({
				item: "milestones",
				ok: true,
				detail: `${files.length} artifacts`,
			});
		} catch (_) {
	...
	if (fs.existsSync(wtDir)) {
		try {
			...
			checks.push({
				item: "worktrack artifacts",
				ok: true,
				detail: `${wts.length} files`,
			});
		} catch (_) {
			checks.push({
				item: "worktrack artifacts",
...
async function guidedRuntimeMigration(rl, state) {
	...
	if (
		summary.state === "no-runtime" ||
		summary.state === "destination-only" ||
		summary.state === "already-migrated"
	) {
		return true;
	}
	...
	if (summary.issue_count > 0 || summary.action === "blocked") {
		console.log(
			`${SYM_FAIL} Runtime migration is blocked; guided install/update will not continue.`,
		);
		console.log(
			`  ${colorDim("Preserve .aw, resolve the reported issue, then rerun guided install/update or migrate-runtime.")}`,
		);
		return false;
	}
...
async function guidedDiagnose(rl, state) {
	...
	for (const bk of diag.backends) {
		...
		console.log(
			`  ${icon} ${colorCyan(bk.backend)}: ${bk.skillCount} skills installed, ${bk.issueCount} issues, ${bk.conflictCount} conflicts`,
		);
		...
		if (!bk.ok && bk.issues.length > 0) {
			for (const iss of bk.issues.slice(0, 3)) {
				console.log(
					`     ${colorDim("- " + (iss.skill || iss.code || JSON.stringify(iss)))}`,
				);
			}
			if (bk.issues.length > 3)
				console.log(
					`     ${colorDim("... and " + (bk.issues.length - 3) + " more")}`,
				);
		}
	}
...
async function guidedPreviewPaths(rl, state) {
	...
	if (totalConflicts > 0) {
		...
		const existingLines = allLines.filter((l) =>
			l.includes("existing target path"),
		);
		if (existingLines.length > 0) {
			...
			for (const cl of existingLines.slice(0, 4)) {
				...
				const compact =
					parts.length > 3 ? ".../" + parts.slice(-3).join("/") : short;
				console.log(`  ${colorDim("  " + compact)}`);
			}
			if (existingLines.length > 4) {
				console.log(
					`  ${colorDim("  ... and " + (existingLines.length - 4) + " more")}`,
				);
			}
		}
...
async function guidedInstall(rl, state) {
	...
	if (status !== 0) {
		...
		if (state.backend === bundleBackend) {
			console.log(
				`${SYM_WARN} Bundle partial: try single-backend recovery with ${colorYellow("servo-installer install --backend agents")} or ${colorYellow("--backend claude")}.`,
			);
		}
		return false;
	}
...
async function guidedVerify(rl, state) {
	...
	if (status !== 0) {
		console.log(
			`${SYM_FAIL} Verification failed. Summary will be marked incomplete.`,
		);
		return false;
	}
...
async function showRecoveryMenu(rl, state, failedStage, detail) {
	...
	if (state.backend === bundleBackend && failedStage === "install") {
		console.log(
			"  3. Retry single backend — " +
				colorYellow("servo-installer install --backend agents"),
		);
		console.log(
			"  4. Retry single backend — " +
				colorYellow("servo-installer install --backend claude"),
		);
	}
	console.log("  c. Cancel and return to menu");
	const maxOpt =
		state.backend === bundleBackend && failedStage === "install" ? "4" : "2";
	const choice = (
		await question(
			rl,
...
async function guidedSummary(rl, state, results) {
	...
	if (ok) {
		...
		console.log(
			"  Verify:     " + statusSymbol(results[4]) + " " + state.verifyResult,
		);
		console.log("\n  " + SYM_OK + " " + colorGreen("Installation ready."));
		if (state.backend === bundleBackend) {
			console.log(
				"  " + colorDim("Both agents and claude backends are deployed."),
			);
		}
		console.log(
			"  " +
				colorDim(
					"Legacy target dir cleanup uses the same update flow: servo-installer update --backend " +
						state.backend +
						" --yes",
				),
		);
		...
		console.log(
			SYM_FAIL + " " + colorBold("Guided flow incomplete — partial state."),
		);
		var stageNames = ["diagnose", "preview", "confirm", "install", "verify"];
		results.forEach((pass, i) => {
			console.log(
				"  " + statusSymbol(pass) + " Stage " + (i + 1) + ": " + stageNames[i],
			);
		});
		...
		if (!results[4]) {
			console.log(
				"  → " +
					colorYellow("servo-installer verify --backend " + state.backend) +
					"  (re-verify)",
			);
		}
...
async function runGuidedFullFlow(rl, state) {
	...
	if (!results[1]) {
		const action = await showRecoveryMenu(
			rl,
			state,
			"preview",
...
async function runGuidedUpdateFlow(rl, state) {
	...
	const diagnoseStatus = await runNodeOwned([
		"diagnose",
		"--backend",
		backend,
		"--json",
	...
	if (diagnoseStatus !== 0) {
		console.log(
			`${SYM_WARN} Diagnose found issues — update may not succeed as expected.`,
		);
		const proceed = (
			await question(
				rl,
...
async function runTuiServoReconcileFlow(rl, state) {
	...
	if (dryRunStatus !== 0) {
		console.log(
			`${SYM_FAIL} .servo template reconcile dry-run failed; not applying.`,
		);
		await pause(rl);
		return;
	}
	...
	if (confirmation !== "yes") {
		console.log(
			".servo template reconciliation cancelled. No changes applied.",
		);
		await pause(rl);
		return;
	}
	...
	if (verifyStatus === 0) {
		console.log(
			`${SYM_OK} .servo template reconciliation verification completed.`,
		);
	} else {
		console.log(
			`${SYM_FAIL} .servo template reconciliation verification failed.`,
		);
	}
	await pause(rl);
}
...
function renderStatusBar(state) {
	...
	const version =
		state.version.length > 20
			? state.version.slice(0, 19) + "…"
			: state.version;
	const repo =
		state.targetRepo.length > width - 18
	...
	process.stdout.write(
		`${CSI_SAVE_CURSOR}${csiCursorTo(0, 0)}${CSI_HIDE_CURSOR}${lines
...
function writeContent(text) {
	process.stdout.write(
		`${CSI_SAVE_CURSOR}${csiCursorTo(STATUS_LINES + 1, 0)}${csiEraseToEnd()}${text}${CSI_RESTORE_CURSOR}`,
	);
}
		...
		try {
			...
			while (true) {
				...
				if (idx === 0) {
					...
					const vStatus = await runNodeOwned([
						"verify",
						"--backend",
						tuiState.backend,
					]);
