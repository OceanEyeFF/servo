...
- 目标分发入口是 `npx servo-installer`，并应支持 CLI + TUI 双模式；当前提供 root package envelope、CLI surface 和最小 `tui` shell，TUI guided update flow 只能作为同一 deploy 合同上的交互式引导层
